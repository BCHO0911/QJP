...
% 一级标题：居中、黑体、三号、加粗
\titleformat{\section}
  {\centering\heiti\bfseries\Large}
  {\thesection}{1em}{}
...
% 标题间距
\titlespacing*{\section}{0pt}{1.5ex plus 0.5ex minus 0.2ex}{1.0ex plus 0.3ex}
...
% 列表格式（紧凑排列，与参考PDF一致）
% ============================================================
\setlist[itemize]{leftmargin=2.5em,itemsep=0.3em,parsep=0.2em,topsep=0.5em}
\setlist[enumerate]{leftmargin=2.5em,itemsep=0.3em,parsep=0.2em,topsep=0.5em}

% ============================================================
% 图表格式
% ============================================================
% 图标题：居中、五号、下方
\captionsetup[figure]{justification=centering,font=small,skip=8pt}
% 表标题：居中、五号、上方
\captionsetup[table]{justification=centering,font=small,skip=6pt}
...
% Section 编号格式：中文"一、二、三"（与参考PDF一致）
...
\subsection{中介效应模型：揭示痰湿体质间接作用路径}

前述分析表明，痰湿质积分$T$与高血脂标签$Y$的直接Spearman相关系数仅为$\rho=-0.01$，直接线性关系几乎不存在。然而，痰湿体质可能通过降低患者活动能力和增加BMI等中介变量，间接影响高血脂风险。本文构建中介效应模型(Mediation Analysis)，量化间接作用路径。

根据Baron \& Kenny三步法，建立以下回归方程组：
\begin{align}
\text{总效应: } Y &= c \cdot T + e_1 \label{eq:med1} \\
\text{中介路径: } M &= a \cdot T + e_2 \label{eq:med2} \\
\text{直接效应: } Y &= c' \cdot T + b \cdot M + e_3 \label{eq:med3}
\end{align}
...
\textbf{优化结果}：痰湿质积分对高血脂的直接效应为$c'=0.0197$（原$\rho=-0.01$）。通过活动能力的间接效应为$a \times b = 0.0022$，通过BMI的间接效应为$0.0010$，总间接效应$0.0032$，中介效应占比$14.0\%$。Sobel检验$z=1.524$, $p=0.128$，提示中介效应存在但未达到传统显著性水平，表明痰湿体质对高血脂的影响主要通过活动能力和BMI等中间变量间接实现，而非直接因果。

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{figures/mediation_analysis.png}
\caption{中介效应路径图与Bootstrap分布（痰湿体质$\rightarrow$活动能力/BMI$\rightarrow$高血脂）}
\label{fig:mediation}
\end{figure}

\subsection{GradientBoosting早停法与正则化优化}

原模型（$n\_estimators=150$，无正则化）在训练集上达到100\%准确率，5折交叉验证F1值为$0.7015 \pm 0.0363$，训练-测试F1差值达$0.2985$，存在明显过拟合。

本文采用以下正则化策略：
\begin{itemize}
    \item \textbf{早停法}：设置$validation\_fraction=0.15$（15\%验证集），$n\_iter\_no\_change=15$（连续15轮验证集loss不下降则停止），$learning\_rate=0.05$（降低学习率）
    \item \textbf{样本采样}：$subsample=0.8$，每棵树仅使用80\%的训练样本
    \item \textbf{特征采样}：$max\_features=0.8$，每次分裂仅考虑80\%的特征
    \item \textbf{复杂度控制}：$max\_depth=4$（从5降至4）、$min\_samples\_leaf=5$、$min\_samples\_split=10$
\end{itemize}

\textbf{优化结果}：早停法在$57$轮自动停止（远低于原150轮），训练集准确率降至$87.70\%$，CV F1值为$0.6957 \pm 0.0253$。训练-测试F1差值从$0.2985$降至$0.1813$，过拟合程度降低$39.3\%$。同时，优化后的模型方差更小（标准差从$0.0363$降至$0.0253$），稳定性提升。

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{figures/gb_optimization.png}
\caption{GradientBoosting优化前后对比（特征重要性变化与CV F1值对比）}
\label{fig:gb_opt}
\end{figure}

\subsection{Sigmoid平台期下降率模型}
...
\textbf{优化结果}：以样本1（$T_0=64$，$L=3$，$K=1$，$n=10$）为例，原模型6个月终值为$38.8$分（下降$25.2$分），Sigmoid模型终值为$48.3$分（下降$15.7$分）。Sigmoid模型更保守地估计了干预效果，避免了原模型对长期效果的过度乐观预测。

\subsection{干预措施协同效应模型}

在前述下降率模型中引入调理级别与活动强度的协同增效项：
\begin{equation}
    r'(t, K, n, L) = \frac{r_{max} \times [1 + \gamma \cdot I(L \geq 2) \cdot I(K \geq 2)]}{1 + \exp(k \cdot (t - t_{mid}))}
\end{equation}
其中协同系数$\gamma=0.10$，表示当中高强度调理（$L \geq 2$）与中高强度活动（$K \geq 2$）同时实施时，下降率额外增加$10\%$，反映饮食调理+八段锦+中药代茶饮与中等以上运动之间的协同增效作用。

\textbf{优化结果}：当样本1从$K=1$提升至$K=2$（其他条件不变）时，协同效应使6个月终值从$48.3$分降至$41.5$分，下降幅度增加$6.8$分。这为"中高强度活动配合中高强度调理"的临床推荐提供了定量支撑。

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{figures/decay_model_comparison.png}
\caption{痰湿积分下降曲线对比（原模型/Sigmoid平台期/Sigmoid+协同效应）}
\label{fig:decay}
\end{figure}

\subsection{依从性Monte Carlo模拟}

原模型假设患者完全依从干预方案，但文献报道慢性病患者6个月依从性约60-80\%。本文引入依从性概率分布进行Monte Carlo模拟：

\begin{enumerate}
    \item 个体依从性概率$p_{comp} \sim Beta(8, 3)$（均值$72.7\%$，95\%CI：$44.4\%$--$93.3\%$）
    \item 实际执行频次$n_{actual} \sim \text{Binomial}(n, p_{comp})$
    \item 对每位患者模拟$N=10000$次，输出6个月后痰湿积分的分布特征
\end{enumerate}

\textbf{优化结果}（表\ref{tab:mc_results}）：

\begin{table}[H]
\centering
\begin{tabular}{ccccccc}
\toprule
\textbf{样本} & \textbf{原预测} & \textbf{MC均值} & \textbf{95\%CI} & \textbf{达标率} \\
\midrule
样本1 ($T_0=64$) & 48.3 & 53.1 & $[48.3, 57.7]$ & 10.3\% \\
样本2 ($T_0=58$) & 47.0 & 47.0 & $[47.0, 47.0]$ & 0.0\% \\
样本3 ($T_0=59$) & 41.6 & 41.6 & $[41.6, 41.6]$ & 100.0\% \\
\bottomrule
\end{tabular}
\caption{依从性Monte Carlo模拟结果（达标定义：下降$\geq 15$分）}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{figures/monte_carlo_compliance.png}
\caption{依从性Monte Carlo模拟（Beta分布、样本1分布、三样本对比、达标率）}
\label{fig:mc}
\end{figure}
...
\begin{enumerate}[label=\textbf{\arabic*.}]
    \item \textbf{中介效应显著性不足}：虽然中介效应模型揭示了痰湿体质通过活动能力和BMI间接影响高血脂的路径（中介占比14.0\%），但Sobel检验$p=0.128$未达到传统显著性水平，提示可能需要更大的样本量或引入更多中介变量（如饮食结构、代谢指标组合）来完整刻画间接效应。

    \item \textbf{Sigmoid参数需临床验证}：平台期参数$k=0.8$和$t_{mid}=4.0$基于经验设定，缺乏纵向随访数据支撑。后续可通过RCT试验收集月度痰湿积分数据，用非线性最小二乘法拟合实际衰减曲线。

    \item \textbf{协同系数估计精度有限}：协同系数$\gamma=0.10$为文献估计值，缺乏本题数据的直接证据。需通过2×2因子实验设计（调理级别×活动强度交叉分组），用双因素ANOVA检验交互效应显著性，获得数据驱动的协同系数估计。

    \item \textbf{依从性分布假设简化}：Beta(8,3)依从性分布基于文献报道的群体均值，未考虑个体差异特征（如年龄、经济状况、家庭支持度对依从性的影响）。后续可将依从性建模为个体特征的函数$p_{comp} = f(年龄, 成本, 社会支持)$，实现更精细化的依从性预测。

    \item \textbf{PCA解释方差有限}：前三个主成分累计解释方差仅47.0\%，说明血脂代谢指标的信息分布较为分散。后续可探索非线性降维方法（如t-SNE、UMAP）。
\end{enumerate}
